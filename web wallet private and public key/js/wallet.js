// Web Wallet JavaScript File
// Complete implementation with key generation and wallet functionality
...
// Action Buttons
const regenerateWalletBtn = document.getElementById('regenerateWallet');
...
// BIP39 wordlist for seed phrase generation
const BIP39_WORDLIST = [
    ...
    'begin', 'behave', 'behind', 'believe', 'below', 'belt', 'bench', 'betray', 'better', 'between',
    ...
    'casual', 'cat', 'catalog', 'catch', 'category', 'cause', 'ceiling', 'celery', 'cement', 'census',
    ...
    'goddess', 'gold', 'good', 'goose', 'gorilla', 'govern', 'gown', 'grab', 'grace', 'grain',
    ...
    'harvest', 'hat', 'have', 'hawk', 'hazard', 'head', 'heal', 'health', 'heart', 'heavy',
    ...
    'ignore', 'ill', 'illegal', 'illness', 'image', 'imitate', 'immediate', 'immense', 'immune', 'impact',
    ...
    'police', 'pond', 'pony', 'pool', 'poor', 'pop', 'popular', 'portion', 'position', 'possible',
    ...
    'speak', 'speed', 'spell', 'spend', 'sphere', 'spice', 'spider', 'spike', 'spin', 'spirit',
    ...
    'strong', 'struggle', 'student', 'stuff', 'stumble', 'stump', 'stun', 'stupid', 'style', 'subject',
    'submit', 'subway', 'success', 'such', 'sudden', 'suffer', 'sugar', 'suggest', 'suit', 'summer',
    'summit', 'sun', 'sunny', 'sunset', 'super', 'supply', 'supreme', 'sure', 'surface', 'surge',
    ...
    'trial', 'tribe', 'trick', 'trigger', 'trim', 'trip', 'triumph', 'trolley', 'trophy', 'trouble',
    'truck', 'true', 'truly', 'trumpet', 'trust', 'truth', 'try', 'tube', 'tuition', 'tulip',
    ...
    'unlock', 'until', 'unusual', 'unveil', 'update', 'upgrade', 'uphold', 'upright', 'uprising', 'uproar',
    ...
    'vast', 'vault', 'vehicle', 'velvet', 'vendor', 'venue', 'verb', 'verify', 'version', 'very',
    ...
    'void', 'volcano', 'volume', 'vote', 'vow', 'vulnerable', 'waddle', 'wagon', 'wait', 'walk',
    'wall', 'walnut', 'want', 'warfare', 'warm', 'warrior', 'wash', 'wasp', 'waste', 'water',
    'waterfall', 'wave', 'way', 'wealth', 'weapon', 'wear', 'weasel', 'weather', 'web', 'wedding',
    'weekend', 'weird', 'welcome', 'west', 'wet', 'whale', 'what', 'whatever', 'wheat', 'wheel',
    'when', 'whenever', 'where', 'whereas', 'wherever', 'whether', 'which', 'whiff', 'while', 'whine',
    'whip', 'whisper', 'wide', 'width', 'wife', 'wild', 'will', 'willing', 'win', 'window',
...
function initializeWallet() {
    ...
    backToWelcomeBtn.addEventListener('click', showWelcomeSection);
    copyPublicKeyBtn.addEventListener('click', () => copyToClipboard(currentWallet.publicKey));
    copyPrivateKeyBtn.addEventListener('click', () => copyToClipboard(currentWallet.privateKey));
    copySeedPhraseBtn.addEventListener('click', () => copyToClipboard(currentWallet.seedPhrase.join(' ')));
    regenerateWalletBtn.addEventListener('click', generateNewWallet);
...
function createWallet() {
    try {
        ...
        createWalletBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Creating Wallet...';
        
        // Generate new wallet
        currentWallet = generateNewWalletData();
        ...
        // Show wallet section and hide welcome
        showWalletSection();
        ...
        showToast('Error creating wallet. Please try again.', 'error');
    } finally {
        // Reset button state
        createWalletBtn.disabled = false;
        createWalletBtn.innerHTML = '<i class="fas fa-plus"></i> Create New Wallet';
    }
}

function generateNewWalletData() {
    ...
    // Generate public key (simplified - in real implementation use proper crypto)
    const publicKey = '04' + privateKey.substring(0, 64);
...
function generateSeedPhrase() {
    ...
    for (let i = 0; i < 12; i++) {
        ...
        const randomIndex = new DataView(randomBytes.buffer).getUint32(0) % BIP39_WORDLIST.length;
...
function displayWalletInfo(wallet) {
    // Display public key
    publicKeyDisplay.innerHTML = `<span>${wallet.publicKey}</span>`;
    copyPublicKeyBtn.style.display = 'block';
    
    // Display private key
    privateKeyDisplay.innerHTML = `<span>${wallet.privateKey}</span>`;
    copyPrivateKeyBtn.style.display = 'block';
    
    // Display seed phrase
    seedPhraseDisplay.innerHTML = wallet.seedPhrase
        .map(word => `<span class="seed-word">${word}</span>`)
        .join('');
    copySeedPhraseBtn.style.display = 'block';
}
...
function generateNewWallet() {
    if (confirm('Are you sure you want to generate a new wallet? This will replace your current wallet.')) {
        try {
            ...
            showToast('Error generating new wallet. Please try again.', 'error');
        }
    }
}
...
function exportWallet() {
    if (!currentWallet) {
        showToast('No wallet to export', 'error');
        return;
    }
    ...
    try {
        ...
        const exportData = {
            ...
            seedPhrase: currentWallet.seedPhrase,
        ...
        a.href = url;
        a.download = `wallet-${new Date().toISOString().split('T')[0]}.json`;
        ...
        showToast('Error exporting wallet. Please try again.', 'error');
    }
}
...
function validateWallet() {
    ...
    return currentWallet.publicKey && 
           ...
           currentWallet.seedPhrase.length === 12;
}

// Security considerations:
// - Never store private keys in localStorage without encryption
...
function startSession() {
    ...
    sessionStorage.setItem('walletSession', 'active');
    sessionBtn.innerHTML = '<i class="fas fa-sign-out-alt"></i>';
...
function endSession() {
    // End wallet session and clear sensitive data
    clearWalletData();
    sessionStorage.removeItem('walletSession');
    sessionBtn.innerHTML = '<i class="fas fa-user-shield"></i>';
    sessionBtn.title = 'Session Management';
    showWelcomeSection();
}

function isSessionActive() {
    // Check if wallet session is active
    return sessionStorage.getItem('walletSession') === 'active';
}

// Enhanced wallet validation
function validateWalletIntegrity() {
    if (!currentWallet) return false;
    
    // Check if all required fields exist
    const hasRequiredFields = currentWallet.publicKey && 
                             currentWallet.privateKey && 
                             currentWallet.seedPhrase;
    ...
    if (currentWallet.seedPhrase.length !== 12) return false;
    
    // Validate all words are in BIP39 wordlist
    const validWords = currentWallet.seedPhrase.every(word => 
...
function handleWalletError(error, operation) {
    ...
    if (error.name === 'QuotaExceededError') {
        ...
        userMessage = 'This operation is not supported in your browser.';
...
// Debounced wallet operations
const debouncedCreateWallet = debounce(createWallet, 300);
const debouncedRegenerateWallet = debounce(generateNewWallet, 300);

// Initialize enhanced event listeners
function initializeEnhancedWallet() {
    // Add debounced event listeners
    createWalletBtn.addEventListener('click', debouncedCreateWallet);
    regenerateWalletBtn.addEventListener('click', debouncedRegenerateWallet);
    
    // Add session management
    if (isSessionActive()) {
        // Restore wallet session if available
        const savedWallet = sessionStorage.getItem('walletData');
        if (savedWallet) {
            try {
